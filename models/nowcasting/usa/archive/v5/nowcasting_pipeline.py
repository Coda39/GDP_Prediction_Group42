"""
GDP Nowcasting Pipeline - V5 (Clean Baseline)
==============================================
This script trains and evaluates multiple models for nowcasting current quarter GDP growth.

VERSION 5 CHANGES (October 30, 2025):
- Fresh start with clean features - NO GDP components
- Removed all GDP-derived features (investment, consumption, household consumption)
- Simple, interpretable feature set: 13 core economic indicators
- Focus: Establish honest baseline performance before adding complex features
- Training: Full historical data (1981-2018, 152 quarters)
- Status: Baseline established with Test R² ≈ 0.01 (essentially predicting mean)

V5 PHILOSOPHY:
Goal: Create a "clean slate" model using only features that are:
1. NOT derived from GDP itself (no circular prediction)
2. Available in real-time for nowcasting
3. Standard economic indicators with clear interpretation
4. Proven to have economic meaning

V5 FEATURE SET (13 features):
Leading Indicators (4):
  - industrial_production_index: Factory output (leading indicator)
  - stock_market_index: Market expectations (forward-looking)
  - interest_rate_short_term: Fed policy stance
  - capital_formation: Investment activity

Coincident Indicators (6):
  - employment_level: Job market size
  - unemployment_rate: Job market health
  - cpi_annual_growth: Inflation rate
  - exports_volume: International trade
  - imports_volume: Domestic demand signal
  - trade_balance: External sector health

Regime Features (3):
  - stock_volatility_4q: Market uncertainty
  - inflation_momentum: Inflation acceleration
  - high_inflation_regime: Binary regime flag

PERFORMANCE RESULTS:
- Training Period: 1981-2018 (152 quarters)
- Test Period: 2022-2025 (12 quarters)
- V5 Test R²: 0.009 (Random Forest) - essentially zero
- Conclusion: Simple indicators alone cannot predict post-COVID period
- Lesson: Need forward-looking financial market indicators (→ V6)

KEY LEARNINGS:
1. Clean features (no GDP components) = honest but weak predictions
2. Traditional economic indicators don't capture 2022-2025 dynamics
3. R² ≈ 0 means model just predicts mean GDP growth (~2.5%)
4. Established need for financial market indicators (credit spreads, yield curve)
5. V5 serves as conservative baseline for comparing V6 improvements

Models:
1. Linear Regression (Baseline)
2. Ridge Regression (L2 Regularization)
3. LASSO Regression (L1 Regularization, Feature Selection)
4. Random Forest Regressor (BEST: R² = 0.009)
5. XGBoost Regressor
...
REGIME = [
    ...
    'inflation_momentum',         # Inflation-based, not GDP
    'high_inflation_regime',      # Inflation-based, not GDP
]

# Combine for nowcasting feature set
NOWCAST_FEATURES = LEADING_INDICATORS + COINCIDENT_INDICATORS + REGIME 
