...
# NEW V6: Financial Market Indicators (NOT GDP components!)
FINANCIAL_INDICATORS_V6 = [
...
# Really good but maybe overfitting
REAL_ACTIVITY_INDICATORS_V6 = [
    'housing_starts',             # Housing activity  
]



# Combine all V6 features
V6_FEATURES = FINANCIAL_INDICATORS_V6 + REAL_ACTIVITY_INDICATORS_V6 


# Complete nowcasting feature set (V1-V5 + V6)
NOWCAST_FEATURES = LEADING_INDICATORS + COINCIDENT_INDICATORS + REGIME + FINANCIAL_INDICATORS_V6
...
class GDPNowcastingPipeline:
    ...
    def load_and_prepare_data(self, country):
        ...
        V6 UPDATE: Now loads extended data (1980-2024) with financial indicators
        ...
        self.log(f"    - V6 financial features: {len([f for f in available_features if f in V6_FEATURES])}")
        ...
        # Check V6 features
        v6_in_data = [f for f in V6_FEATURES if f in feature_cols]
        v6_missing = [f for f in V6_FEATURES if f not in feature_cols]
        self.log(f"\nV6 features present: {len(v6_in_data)}/{len(V6_FEATURES)}")
        if v6_in_data:
            self.log(f"  Present: {v6_in_data}")
        if v6_missing:
            self.log(f"  MISSING: {v6_missing}")
    ...
    def train_all_models(self, country):
        ...
        self.log(f"\n{'='*70}")
        self.log(f"Training V6 Nowcasting Models: {country}")
        self.log(f"{'='*70}")

        # Load data
        self.log("[Step 1/3] Loading and preparing V6 data (with financial indicators)...")
        ...
        for model_name, model in models.items():
            ...
            # Save model
            model_file = MODEL_DIR / f'{country.lower()}_nowcast_v6_{model_name.lower().replace(" ", "_")}.pkl'
    ...
    def print_results_summary(self, country):
        ...
        self.log(f"\n{'='*70}")
        self.log(f"V6 Nowcasting Results Summary: {country}")
        self.log(f"{'='*70}")
        ...
        # Save to CSV
        summary_df.to_csv(OUTPUT_DIR / f'{country.lower()}_nowcast_v6_results.csv', index=False)
    ...
    def plot_predictions(self, country):
        ...
        plt.suptitle(f'{country} - V6 GDP Nowcasting Results (with Financial Indicators)\nPredictions vs Actuals',
    ...
    def plot_feature_importance(self, country):
        ...
        plt.suptitle(f'{country} - V6 Nowcasting Feature Importance',
                    fontsize=14, fontweight='bold')
        plt.tight_layout()
    ...
    def plot_time_series_predictions(self, country):
        ...
        if not np.isnan(best_metrics['test']['RMSE']):
            ...
            ax.set_title(f'{country} - V6 Nowcasting Time Series (with Financial Indicators)\nBest Model: {best_model_name} (Test R² = {best_metrics["test"]["R2"]:.3f})',
                        fontsize=14, fontweight='bold')
...
def main():
    """Main execution"""
    print("="*80)
    print("GDP NOWCASTING PIPELINE V6 - WITH FINANCIAL INDICATORS")
    print("="*80)
    ...
    print("V6 Target:   Test R² > 0.10 (180 quarters, with financial indicators)")
    print("="*80)
    ...
    for country in FOCUS_COUNTRIES:
        ...
        if results:
            ...
            print(f"\nGenerating V6 visualizations for {country}...")
    ...
    print(f"\n{'='*80}")
    print("V6 Cross-Country Nowcasting Summary")
    print(f"{'='*80}")
    ...
    combined_df.to_csv(OUTPUT_DIR / 'all_countries_nowcast_v6_results.csv', index=False)

    # Print best models
    if len(combined_df) > 0:
        print("\nBest V6 Model by Country (Test R²):")
    ...
    print("\n" + "="*80)
    print("V6 NOWCASTING PIPELINE COMPLETE!")
    print("="*80)
    print(f"Key Changes from V5:")
    print(f"  - Training data: 72 quarters (2000-2018) → 180 quarters (1980-2024)")
    print(f"  - Features: ~13 baseline → ~30 with {len(V6_FEATURES)} financial features")
