...
NOWCAST_FEATURES = (
    LEADING_INDICATORS + COINCIDENT_INDICATORS + REGIME + FINANCIAL_INDICATORS
)
...
class GDPNowcastingPipeline:
    ...
    def load_and_prepare_data(self, country):
        ...
        df = df.drop(
            columns=[col for col in DROP_FEATURES if col in df.columns], errors="ignore"
        )
        ...
        self.log(
            f"    - Core features (V1-V5): {len(LEADING_INDICATORS) + len(COINCIDENT_INDICATORS) + len(REGIME)}"
        )
        self.log(
            f"    - financial features: {len([f for f in available_features if f in FINANCIAL_INDICATORS])}"
        )
        ...
        train_df[feature_cols] = (
            train_df[feature_cols].fillna(method="ffill").fillna(method="bfill")
        )
        val_df[feature_cols] = (
            val_df[feature_cols].fillna(method="ffill").fillna(method="bfill")
        )
        test_df[feature_cols] = (
            test_df[feature_cols].fillna(method="ffill").fillna(method="bfill")
        )
    ...
    def train_all_models(self, country):
        ...
        stacking = StackingRegressor(
            estimators=estimators, final_estimator=Ridge(alpha=1.0), cv=3, n_jobs=-1
        )
        ...
        for model_name, model in models.items():
            ...
            model_file = (
                MODEL_DIR
                / f"{country.lower()}_nowcast_v7_{model_name.lower().replace(' ', '_')}.pkl"
            )
        ...
        for model_name in ["Random Forest", "XGBoost", "Gradient Boosting"]:
            if hasattr(models[model_name], "feature_importances_"):
                ...
                self.log(
                    self.feature_importance[country][model_name]
                    .head(10)
    ...
    def print_results_summary(self, country):
        ...
        for model_name, metrics in results.items():
            row = {
                ...
                "Val RMSE": f"{metrics['val']['RMSE']:.3f}"
                if not np.isnan(metrics["val"]["RMSE"])
                else "N/A",
                "Test RMSE": f"{metrics['test']['RMSE']:.3f}"
                if not np.isnan(metrics["test"]["RMSE"])
                else "N/A",
                "Train R²": f"{metrics['train']['R2']:.3f}",
                "Val R²": f"{metrics['val']['R2']:.3f}"
                if not np.isnan(metrics["val"]["R2"])
                else "N/A",
                "Test R²": f"{metrics['test']['R2']:.3f}"
                if not np.isnan(metrics["test"]["R2"])
                else "N/A",
            }
        ...
        # Save to CSV
        summary_df.to_csv(
            OUTPUT_DIR / f"{country.lower()}_nowcast_V7_results.csv", index=False
        )
    ...
    def plot_predictions(self, country):
        ...
        plt.suptitle(
            f"{country} - V7 GDP Nowcasting Results (with Financial Indicators)\nPredictions vs Actuals",
            fontsize=14,
            fontweight="bold",
        )
        plt.tight_layout()
        plt.savefig(
            FIG_DIR / f"{country.lower()}_nowcast_predictions.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def plot_feature_importance(self, country):
        ...
        if (
            country not in self.feature_importance
            or not self.feature_importance[country]
        ):
            return
        ...
        for idx, (model_name, importance_df) in enumerate(
            self.feature_importance[country].items()
        ):
            ax = axes[idx]
        ...
        plt.suptitle(
            f"{country} - Nowcasting Feature Importance", fontsize=14, fontweight="bold"
        )
        plt.tight_layout()
        plt.savefig(
            FIG_DIR / f"{country.lower()}_nowcast_feature_importance.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        ...
        self.log(
            f"  ✓ Saved feature importance plot: {country}_nowcast_feature_importance.png"
        )
    ...
    def plot_time_series_predictions(self, country):
        ...
        best_model_name = max(
            results.keys(),
            key=lambda k: results[k]["test"]["R2"]
            if not np.isnan(results[k]["test"]["R2"])
            else -np.inf,
        )
        ...
        if not np.isnan(best_metrics["test"]["RMSE"]):
            ...
            ax.set_title(
                f"{country} - Nowcasting Time Series (with Financial Indicators)\nBest Model: {best_model_name} (Test R² = {best_metrics['test']['R2']:.3f})",
                fontsize=14,
                fontweight="bold",
            )
            ...
            plt.tight_layout()
            plt.savefig(
                FIG_DIR / f"{country.lower()}_nowcast_timeseries.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
...
def main():
    ...
    if len(combined_df) > 0:
        ...
        best_models = combined_df.loc[
            combined_df.groupby("Country")["Test_R2"].idxmax()
        ]
        print(
            best_models[["Country", "Model", "Test_R2", "Test_RMSE"]].to_string(
                index=False
            )
        )
    ...
    print(
        f"  - Features: ~13 baseline → ~30 with {len(FINANCIAL_INDICATORS)} financial features"
    )
