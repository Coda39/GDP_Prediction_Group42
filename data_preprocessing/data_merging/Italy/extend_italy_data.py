...
# FRED series for Italy - ALL economic indicators
FRED_SERIES = {
    # GDP
    'gdp_real': 'CLVMNACSCAB1GQIT',  # Real GDP (billions of 2015 euros, quarterly)
    'gdp_nominal': 'CPMNACSCAB1GQIT',  # Nominal GDP (quarterly)
    
    # Labor market
    'unemployment_rate': 'LRHUTTTTITQ156S',  # Unemployment rate
    'employment_level': 'LMEMTTTTITQ659S',  # Employment level (thousands)
    
    # Prices
    'cpi_all_items': 'ITACPIALLMINMEI',  # CPI all items (monthly → quarterly)
    
    # Trade
    'exports_volume': 'XTEXVA01ITQ188S',  # Exports volume index
    'imports_volume': 'XTIMVA01ITQ188S',  # Imports volume index
    
    # Production
    'industrial_production_index': 'ITAPROINDMISMEI',  # Industrial production
    
    # Financial
    'interest_rate_short_term': 'IRSTCI01ITM156N',  # Short-term interest rate
    'interest_rate_long_term': 'IRLTLT01ITM156N',  # Long-term interest rate
    'stock_market_index': 'FTSEMIB',  # FTSE MIB index
    
    # Other
    'consumer_confidence': 'CSCICP03ITM665S',  # Consumer confidence
}
...
BLOOMBERG_FILES = {
    ...
    'gov_2y': BLOOMBERG_DIR / 'Italy Gov 2Y.xlsx',
    'gov_5y': BLOOMBERG_DIR / 'Italy Gov 5Y.xlsx',
    'gov_10y': BLOOMBERG_DIR / 'Italy Gov 10Y.xlsx',
}
...
def download_fred_historical():
    """Download ALL available FRED data for Italy (1980-2025)"""
    print("="*80)
...
def load_base_data():
    """Load current Italy data (2000-2025)"""
    print("\n" + "="*80)
...
def main():
    print("="*80)
    print(" ITALY DATA EXTENSION - FRED HISTORICAL + BLOOMBERG")
    print("="*80)
