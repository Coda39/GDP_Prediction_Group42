"""
GDP Quarterly Forecasting Pipeline
===================================
This script trains and evaluates multiple models for forecasting GDP growth 1-4 quarters ahead.
...
Target: GDP Growth (YoY %) for next 1-4 quarters
...
# Features to drop (M2 missing for some, not critical)
DROP_FEATURES = ['money_supply_broad', 'm2_growth', 'country']

# Target variable
TARGET = 'gdp_growth_yoy'

class GDPForecastingPipeline:
    """Complete forecasting pipeline for GDP growth"""

    def __init__(self, horizon=1, verbose=True):
        """
        Initialize forecasting pipeline

        Parameters:
        - horizon: forecast horizon in quarters (1, 2, 3, or 4)
    ...
    def load_and_prepare_data(self, country):
        ...
        # Create future target (shift target back by horizon)
        df[f'{TARGET}_future'] = df[TARGET].shift(-self.horizon)

        # Drop rows where future target is NaN
        ...
        # Select features (exclude target and future target)
        feature_cols = [col for col in df.columns if col not in [TARGET, f'{TARGET}_future']]

        # Handle remaining missing values (drop features with >30% missing, impute rest)
        ...
        y_train = train_df[f'{TARGET}_future']

        X_val = val_df[feature_cols] if len(val_df) > 0 else pd.DataFrame()
        y_val = val_df[f'{TARGET}_future'] if len(val_df) > 0 else pd.Series()

        X_test = test_df[feature_cols] if len(test_df) > 0 else pd.DataFrame()
        y_test = test_df[f'{TARGET}_future'] if len(test_df) > 0 else pd.Series()
    ...
    def train_all_models(self, country):
        ...
        self.log(f"\n{'='*70}")
        self.log(f"Training Models: {country} | Horizon: {self.horizon} quarter(s)")
        self.log(f"{'='*70}")
        ...
        for model_name, model in models.items():
            ...
            # Save model
            model_file = MODEL_DIR / f'{country.lower()}_h{self.horizon}_{model_name.lower().replace(" ", "_")}.pkl'
    ...
    def print_results_summary(self, country):
        ...
        self.log(f"\n{'='*70}")
        self.log(f"Results Summary: {country} (Horizon: {self.horizon}Q)")
        self.log(f"{'='*70}")
        ...
        # Save to CSV
        summary_df.to_csv(OUTPUT_DIR / f'{country.lower()}_h{self.horizon}_results.csv', index=False)
    ...
    def plot_predictions(self, country):
        ...
        plt.suptitle(f'{country} - GDP Forecasting Results (Horizon: {self.horizon}Q)\nPredictions vs Actuals',
                    fontsize=14, fontweight='bold')
        plt.tight_layout()
        plt.savefig(FIG_DIR / f'{country.lower()}_h{self.horizon}_predictions.png', dpi=300, bbox_inches='tight')
        plt.close()

        self.log(f"  ✓ Saved predictions plot: {country}_h{self.horizon}_predictions.png")
    ...
    def plot_feature_importance(self, country):
        ...
        plt.suptitle(f'{country} - Feature Importance (Horizon: {self.horizon}Q)',
                    fontsize=14, fontweight='bold')
        plt.tight_layout()
        plt.savefig(FIG_DIR / f'{country.lower()}_h{self.horizon}_feature_importance.png', dpi=300, bbox_inches='tight')
        plt.close()

        self.log(f"  ✓ Saved feature importance plot: {country}_h{self.horizon}_feature_importance.png")

def main():
    """Main execution"""
    print("="*80)
    print("GDP QUARTERLY FORECASTING PIPELINE")
    print("="*80)

    # Forecast horizons to test
    horizons = [1, 4]  # 1 quarter and 4 quarters (1 year) ahead

    for horizon in horizons:
        print(f"\n{'='*80}")
        print(f"FORECASTING HORIZON: {horizon} QUARTER(S) AHEAD")
        print(f"{'='*80}")

        pipeline = GDPForecastingPipeline(horizon=horizon, verbose=True)
        ...
        print(f"\n{'='*80}")
        print(f"Cross-Country Summary (Horizon: {horizon}Q)")
        print(f"{'='*80}")
        ...
        combined_df.to_csv(OUTPUT_DIR / f'all_countries_h{horizon}_results.csv', index=False)
